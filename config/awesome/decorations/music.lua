...
local music_toolbar_toggle = function(c)
...
local create_toolbar_button = function(c)
    ...
    toolbar_button:buttons(gears.table.join(
        awful.button({ }, 1, function() music_toolbar_toggle(c) end)
    ))
...
local music_buttons = require("noodle.music_buttons")
...
-- Update bar
awesome.connect_signal("evil::music_volume", function(value)
...
awesome.connect_signal("evil::music_options", function(loop, random)
    if loop then
...
notifications_button = control_button(c, notifications_symbol, notifications.music.enabled and notifications_color or disabled_color, dpi(30), function()
    notifications.music.toggle()
    local text = notifications_button:get_all_children()[1]:get_all_children()[1]
    text.markup = helpers.colorize_text(notifications_symbol, notifications.music.enabled and notifications_color or disabled_color)
end)
        ...
        {
            {
                ...
                control_button(c, "", x.color5, dpi(30),
                    ...
                    function()
                        awful.spawn.with_shell("mpc toggleoutput music_fifo")
                    end
                    ),
                random_button,
                loop_button,
